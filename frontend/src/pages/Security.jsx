...
const Security = () => {
    ...
    const fetchProfile = async () => {
        try {
            ...
            const res = await axios.get((import.meta.env.VITE_API_URL || 'http://localhost:5000') + '/api/profile', {
    ...
    const setupMFA = async () => {
        try {
            const token = localStorage.getItem('token');
            const res = await axios.post((import.meta.env.VITE_API_URL || 'http://localhost:5000') + '/api/mfa/setup', {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const verifyMFA = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.post((import.meta.env.VITE_API_URL || 'http://localhost:5000') + '/api/mfa/verify', { token: otpToken }, {
                headers: { Authorization: `Bearer ${token}` }
            });
            setMfaEnabled(true);
            setStep('overview');
    ...
    const handleSetPin = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post((import.meta.env.VITE_API_URL || 'http://localhost:5000') + '/api/auth/pin/set', { pin }, {
                headers: { Authorization: `Bearer ${token}` }
            });
            setPinSet(true);
            setStep('overview');
    ...
    const handleChangePin = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.post((import.meta.env.VITE_API_URL || 'http://localhost:5000') + '/api/auth/pin/change', { oldPin, newPin }, {
                headers: { Authorization: `Bearer ${token}` }
            });
            setStep('overview');
            setOldPin('');
